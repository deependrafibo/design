import { Meta, StoryObj } from '@storybook/react';
import { ImageInput } from './ImageInput';
import { User } from 'react-feather';

const meta: Meta<typeof ImageInput> = {
  title: 'Components/input/image',
  component: ImageInput,
  parameters: {
    layout: 'centered',
    docs: {
      description: {
        component:
          'A component for uploading and updating an image. It provides a button to select a file, and it supports file validation, tooltips, custom labels, and custom placeholders.',
      },
    },
  },
  tags: ['autodocs'],
};

export default meta;
type Story = StoryObj<typeof ImageInput>;

export const Default: Story = {
  args: {
    onImageSelect: (file: File) => {
      console.log('Image selected:', file);
    },
    updateButtonLabel: 'Update Picture',
    tooltipText: 'Allowed file types: png, jpg, jpeg.',
    allowedFileTypes: ['png', 'jpg', 'jpeg'],
  },
};

export const WithImage: Story = {
  args: {
    onImageSelect: (file: File) => {
      console.log('Image selected:', file);
    },
    onRemoveImage: () => {
      console.log('Image removed');
    },
    currentImageUrl:
      'data:image/jpeg;base64,',
    updateButtonLabel: 'Update Picture',
    editButtonLabel: 'Edit Picture',
    tooltipText: 'Allowed file types: png, jpg, jpeg.',
    allowedFileTypes: ['png', 'jpg', 'jpeg'],
  },
};

export const Disabled: Story = {
  args: {
    onImageSelect: (file: File) => {
      console.log('Image selected:', file);
    },
    disabled: true,
    updateButtonLabel: 'Update Picture',
    tooltipText: 'Allowed file types: png, jpg, jpeg.',
    allowedFileTypes: ['png', 'jpg', 'jpeg'],
  },
};

export const WithCustomTooltip: Story = {
  args: {
    onImageSelect: (file: File) => {
      console.log('Image selected:', file);
    },
    updateButtonLabel: 'Update Picture',
    tooltipText: 'Custom tooltip: Only images of size less than 5MB.',
    allowedFileTypes: ['png', 'jpg', 'jpeg'],
  },
};

export const WithStringPlaceholder: Story = {
  args: {
    onImageSelect: (file: File) => {
      console.log('Image selected:', file);
    },
    updateButtonLabel: 'Update Picture',
    tooltipText: 'Allowed file types: png, jpg, jpeg.',
    allowedFileTypes: ['png', 'jpg', 'jpeg'],
    customPlaceholder: 'Add Photo',
  },
};

export const WithIconPlaceholder: Story = {
  args: {
    onImageSelect: (file: File) => {
      console.log('Image selected:', file);
    },
    updateButtonLabel: 'Update Picture',
    tooltipText: 'Allowed file types: png, jpg, jpeg.',
    allowedFileTypes: ['png', 'jpg', 'jpeg'],
    customPlaceholder: <User size={40} className="text-gray-400" />,
    useTransparentBackground: true,
  },
};

export const WithCustomComponentPlaceholder: Story = {
  args: {
    onImageSelect: (file: File) => {
      console.log('Image selected:', file);
    },
    updateButtonLabel: 'Update Picture',
    tooltipText: 'Allowed file types: png, jpg, jpeg.',
    allowedFileTypes: ['png', 'jpg', 'jpeg'],
    customPlaceholder: (
      <div className="flex flex-col items-center gap-2 text-gray-400">
        <User size={32} />
        <span className="text-sm">Upload Photo</span>
      </div>
    ),
  },
};
