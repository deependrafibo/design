import { ColoredGridMatrixProps } from './types';
import { isEmpty } from 'lodash';

export default function HeatMap({ matrixConfig, matrixData }: ColoredGridMatrixProps) {
  ...
  return (
    <div className="flex flex-col gap-y-4">
          ...
          {matrixConfig?.legend?.map((legendItem, index) => (
            <div key={index} className="flex flex-row gap-x-1 items-center">
              ...
              <div className="text-xs text-grey-700 font-normal leading-5">
                {legendItem?.range_min} - {legendItem?.range_max}
                {legendItem?.legend_category_name && `: ${legendItem?.legend_category_name}`}
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="flex flex-col items-start">
        <div className="grid grid-flow-row auto-rows-auto gap-3 items-center">
          {/* Data rows */}
          {matrixData
            ?.slice()
            ?.reverse()
            ?.map((row, rowIndex) => (
              <div key={rowIndex} className="grid grid-flow-col auto-cols-fr gap-3 items-center">
                {/* Row label */}
                <div className="text-xs text-grey-700 font-normal max-w-28 text-center truncate mr-2">{row?.label}</div>
                ...
                {Object?.entries(row)
                  ...
                  ?.map(([columnName, value]) => {
                    ...
                    const legendItem = matrixConfig?.legend?.find(
                      (item) => score >= item?.range_min && score <= item?.range_max,
                    );
